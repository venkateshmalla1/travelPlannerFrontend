...
import RecommendedHotels from '../RecomendendHotels/RecomendendHotels.js';
import SafetyAndCaustions from '../SafetyAndCautions/SafetyAndCautions.js';
import { tripService } from '../../services/TripService.js';

export default function TravelDetails() {
  const { selectedTrip, setSelectedTrip } = useTravel();
  const { token } = useAuth();
  ...
  const handleAlterAction = async () => {
    if (!instructions || !currentDayData) return;
    setAltering(true);
    try {
      const response = await tripService.modifyDay(
        selectedTrip._id,
        { targetDay: currentDayData.day, changeInstructions: instructions },
        token
      );
      setSelectedTrip(response.refreshedTrip);
      setInstructions('');
    } catch (error) {
      alert(error.message || 'Failed to alter itinerary sequence.');
    } finally {
      setAltering(false);
    }
  };
  ...
  return (
    <div>
      <div className="card shadow-sm border-0 p-4 bg-white text-start">
        ...
        <div className="row">
          <div className="col-lg-8 border-end">
            ...
            <div className="d-flex gap-2 border-top pt-3 mt-4">
              ...
              <button type="button" className="btn btn-sm btn-dark text-nowrap" onClick={handleAlterAction} disabled={altering || !instructions || !currentDayData}>Alter Day</button>
            </div>
          </div>
          ...
          <div className="col-lg-4 mt-4 mt-lg-0">
            <div className="card border bg-light p-3 mb-3">
              ...
              <div className="d-flex justify-content-between text-xs border-bottom py-1 small"><span>Hotels:</span><strong>{selectedTrip.tripSummary.currency}{selectedTrip.budgetBreakdown?.accommodation}</strong></div>
              <div className="d-flex justify-content-between text-xs border-bottom py-1 small"><span>Food:</span><strong>{selectedTrip.tripSummary.currency}{selectedTrip.budgetBreakdown?.food}</strong></div>
              <div className="d-flex justify-content-between text-xs border-bottom py-1 small"><span>Activities:</span><strong>{selectedTrip.tripSummary.currency}{selectedTrip.budgetBreakdown?.activities}</strong></div>
              <div className="d-flex justify-content-between pt-2 fw-bold text-success small"><span>Total Cost:</span><span>{selectedTrip.tripSummary.currency}{selectedTrip.budgetBreakdown?.totalEstimatedBudget}</span></div>
            </div>
            <ThingsToCarry
              carryLists={selectedTrip.thingsToCarry}
              storageKey={`travel-planner:things-to-carry:${selectedTrip._id}`}
            />
          </div>
        </div>
      </div>
      ...
      <div className="mt-4">
        <RecommendedHotels data={selectedTrip} />
      </div>

      <div className="mt-4">
        <SafetyAndCaustions data={selectedTrip} />
      </div>
    </div>
  );
}
